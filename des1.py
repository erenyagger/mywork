...
#вывод информации о размерности набора данных
df.shape
...
# Были добавлены дополнительные атрибуты, которые означают:
# * mean_new_cases - среднее значение новых подтвержденных случаев заболевания COVID-19 по странам
# * mean_new_deaths - среднее значение новых случаев смерти, связанные с COVID-19 по странам

# In[11]:


#вывод значений новых атрибутов
grouped_cases
...
#объединение набора данных с набором дополнительных атрибутов
...
#вывод нескольких первых строк объединённого датасета
df.head()
...
#проверка атрибутов на наличие пустых значений
pd.set_option('display.max_rows',None)
df.isnull().sum()
...
# Добавим дополнительный атрибут месяц, для того, чтобы анализировать количество заболевающих COVID-19 в зависимости от месяца
...
#формироание набора данных, состоящего из атрибутов location и Rt
...
#цикл для построения столбцов на графике
